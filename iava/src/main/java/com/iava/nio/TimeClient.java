...
public class TimeClient {
	...
	protected void sendRequests() throws Exception {
		...
		while (it.hasNext()) {
			...
			System.out.println("Requesting time from " + sa.getHostName() + ":"
					+ sa.getPort());
			// Make it empty (see RFC868) buffer.clear().flip( ); // Fire and
			// forget
	...
	public void getReplies() throws Exception {
		...
		longBuffer.order(ByteOrder.BIG_ENDIAN); // Zero the whole buffer to be
												// sure
		longBuffer.putLong(0, 0); // Position to first byte of the low-order 32
									// bits
		longBuffer.position(4); // Slice the buffer; gives view of the low-order
								// 32 bits
	...
	protected void printTime(long remote1900, InetSocketAddress sa) {
		...
		long local = System.currentTimeMillis() / 1000; // remote time as
														// seconds since Jan 1,
														// 1970
	...
	protected void parseArgs(String[] argv) {
		...
		for (int i = 0; i < argv.length; i++) {
			String arg = argv[i]; // Send client requests to the given port
			if (arg.equals("-p")) {
				i++;
				this.port = Integer.parseInt(argv[i]);
				continue;
			}// Create an address object for the hostname
			InetSocketAddress sa = new InetSocketAddress(arg, port); // Validate
																		// that
																		// it
																		// has
																		// an
