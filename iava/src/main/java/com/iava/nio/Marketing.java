...
public class Marketing {
	...
	public static void main(String[] argv) throws Exception {
		...
		GatheringByteChannel gatherChannel = fos.getChannel(); // Generate some
																// brilliant
																// marcom, er,
																// repurposed
																// content
		ByteBuffer[] bs = utterBS(reps); // Deliver the message to the waiting
											// market
		while (gatherChannel.write(bs) > 0) { // Empty body // Loop until write(
												// )
			// returns zero }
	...
	private static Random rand = new Random(); // Pick one, make a buffer to
												// hold it and the suffix, load
												// it with

	// the byte equivalent of the strings (will not work properly for
	// non-Latin characters), then flip the loaded buffer so it's ready // to be
	// drained
